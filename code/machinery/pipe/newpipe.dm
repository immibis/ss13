...
// sets the vnodes terminators to the joining machines (or null)
/obj/machinery/pipeline/proc/setterm()
	...
	for(var/obj/machinery/pipes/P in nodes)
		if(!P.node1)
			var/list/ndirs = P.get_node_dirs()
			leaks += get_step(P, ndirs[1])
		else if(!P.node1.ispipe() && !(P.node1 in vnodes))
			vnodes += P.node1

		if(!P.node2)
			var/list/ndirs = P.get_node_dirs()
			leaks += get_step(P, ndirs[2])
		else if(!P.node2.ispipe() && !(P.node2 in vnodes))
			vnodes += P.node2

		if(istype(P, /obj/machinery/manifold))
			var/obj/machinery/manifold/M = P
			if(!M.node3)
				var/list/ndirs = M.get_node_dirs()
				leaks += get_step(M, ndirs[3])
			else if(!M.node3.ispipe() && !(M.node3 in vnodes))
				vnodes += M.node3
...
//	var/dbg = (suffix == "d") && Debug
	...
	for(var/obj/machinery/vn in vnodes)
		delta_gt = FLOWFRAC * ( vn.get_gas_val(src) - gas.tot_gas() / capmult)
		calc_delta( src, gas, ngas, vn, delta_gt)//, dbg)
...
/obj/machinery/pipeline/proc/leak_to_turf(turf/T)
	if(T == null)
		return
	if(T.density)
		return
...
/proc/makepipelines()
	...
	for(var/obj/machinery/M in machines)		// look for a pipe
		var/obj/machinery/pipes/P = M
		if(!istype(P, /obj/machinery/pipes) && !istype(P, /obj/machinery/manifold))
			continue
		if(!P.plnum)							// if not already part of a line
			P.buildnodes(++linecount)			// add it, and spread to all connected pipes
			while(buildnodes_queue.len > 0)
				var/list/temp = buildnodes_queue
				buildnodes_queue = new
				for(var/obj/machinery/P2 in temp)
					P2.buildnodes(linecount)
	...
	for(var/obj/machinery/M in machines)		// look for pipes
		var/obj/machinery/pipes/P = M
		if(!istype(P, /obj/machinery/pipes) && !istype(P, /obj/machinery/manifold))
			continue
		var/obj/machinery/pipeline/PL = plines[P.plnum]
		if(PL.numnodes == 0)
	...
	for(var/obj/machinery/manifold/M in machines)
		M.setline()
...
/proc/pipelist(var/obj/machinery/source, var/obj/machinery/startnode)
	...
	var/list/nodes = list(startnode)
	var/list/prevs = list(source)

	while(nodes.len > 0)
		var/obj/machinery/node = nodes[1]
		var/obj/machinery/prev = prevs[1]
		nodes.Cut(1, 2)
		prevs.Cut(1, 2)
		...
		var/list/next = node.next(prev)

		if(next == null)
			continue

		if(istype(next, /list))
			for(var/obj/machinery/M in next)
				if(M && M.ispipe())
					nodes.len = nodes.len + 1
					prevs.len = prevs.len + 1
					nodes[nodes.len] = M
					prevs[prevs.len] = node
		else
			var/obj/machinery/M = next
			if(M && M.ispipe())
				nodes.len = nodes.len + 1
				prevs.len = prevs.len + 1
				nodes[nodes.len] = next
				prevs[prevs.len] = node
...
obj/machinery/manifold/next(var/obj/machinery/from)
	var/list/L = new

	if(node1 && node1.ispipe() && from != node1)
		L += node1
	if(node2 && node2.ispipe() && from != node2)
		L += node2
	if(node2 && node3.ispipe() && from != node3)
		L += node3

	return L

obj/machinery/manifold/setline()
	pl = plines[plnum]

obj/machinery/manifold/ispipe()
	return 1
...
/obj/machinery/pipes/proc/update()
	...
	if(node1 && node2)
		...
		overlays += image('icons/ss13/pipes.dmi', "discon", FLY_LAYER, dirs[1])
		overlays += image('icons/ss13/pipes.dmi', "discon", FLY_LAYER, dirs[2])
	else if(!node1)
...
var/obj/machinery/pipes/buildnodes_queue[] = new
...
/obj/machinery/pipes/heat_exch/buildnodes(var/linenum)
	...
	if(node1 && node1.ispipe() )
		buildnodes_queue += node1
	else
		termination++

	if(node2 && node2.ispipe() )
		buildnodes_queue += node2
	else
		termination++
...
/*obj/machinery/circulator/verb/rebuildnodes()
	set src in view()
	buildnodes()*/
...
obj/machinery/manifold
	New()
		..()
		p_dir = p_dir | turn(dir, 90) | turn(dir, -90)

	getline()
		return pl

	proc/get_node_dirs()
		return list(n1dir, n2dir, dir)

	buildnodes(var/linenum)
		...
		node1 = get_machine(level, src.loc, n1dir)
		node2 = get_machine(level, src.loc, n2dir)
		node3 = get_machine(level, src.loc, dir)
		...
		if(node3 && node3.ispipe() )
			node3.buildnodes(linenum)
...
/proc/Plasma()
	...
	for(var/turf/simulated/engine/floor/T in world)
		tplas += T.poison
...
/obj/machinery/valve/dvalve/proc/toggle()
	if(!open)		// now opening
		flick("dvalve01", src)
		icon_state = "dvalve1"
		sleep(10)
