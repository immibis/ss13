...
/obj/machinery/filter_control/New()
	...
	spawn(5)	//wait for world
		for(var/obj/machinery/atmospherics/unary/inlet/filter/F in machines)
...
/obj/machinery/filter_control/process()
	if(!(stat & NOPOWER))
		use_power(FILTER_CONTROL_POWER,ENVIRON)
		AutoUpdateAI(src)
		src.updateUsrDialog()
	src.updateicon()
	...
	if (((get_dist(src, usr) <= 1 || istype(usr, /mob/ai)) && istype(src.loc, /turf)))
		...
		if (src.allowed(usr) || src.emagged && !(stat & BROKEN))
			if (href_list["tg"])	//someone modified the html so I added a check here
				...
				for(var/obj/machinery/atmospherics/unary/inlet/filter/FI in machines)
...
/obj/machinery/filter_control/proc/updateicon()
	...
	if(src.locked && (stat & BROKEN))
		overlays += image('icons/ss13/stationobjs.dmi', "filter_control00")
		return
	else if(!src.locked)
		icon_state = "filter_control-unlocked"
		if(stat & BROKEN)
			overlays += image('icons/ss13/stationobjs.dmi', "filter_control-wirecut")
			overlays += image('icons/ss13/stationobjs.dmi', "filter_control00")
			return
	...
	if(GoodConnection && src.f_mask)
		overlays += image('icons/ss13/stationobjs.dmi', "filter_control1")
	else if(GoodConnection)
		overlays += image('icons/ss13/stationobjs.dmi', "filter_control10")
	else if(src.f_mask)
		overlays += image('icons/ss13/stationobjs.dmi', "filter_control0")
	else
		overlays += image('icons/ss13/stationobjs.dmi', "filter_control00")

	if (src.f_mask & (GAS_N2O|GAS_PL))
		src.overlays += image('icons/ss13/stationobjs.dmi', "filter_control-tox")
	if (src.f_mask & GAS_O2)
		src.overlays += image('icons/ss13/stationobjs.dmi', "filter_control-o2")
	if (src.f_mask & GAS_N2)
		src.overlays += image('icons/ss13/stationobjs.dmi', "filter_control-n2")
	if (src.f_mask & GAS_CO2)
		src.overlays += image('icons/ss13/stationobjs.dmi', "filter_control-co2")
	return
