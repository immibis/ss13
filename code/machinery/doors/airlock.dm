...
/obj/machinery/door/airlock
	name = "Airlock"
	icon = 'icons/goonstation/obj/doors/doorint.dmi'
...
/obj/machinery/door/airlock
	external
		icon = 'icons/goonstation/obj/doors/doorext.dmi'
		req_access_txt = "open_external_airlocks"
	security
		icon = 'icons/goonstation/obj/doors/doorsec.dmi'
		req_access_txt = "access_security"
	glass
		icon = 'icons/goonstation/obj/doors/doorglass.dmi'
		opacity = 0
		visible = 0
	engineering
		icon = 'icons/goonstation/obj/doors/dooreng.dmi'
		req_access_txt = "access_engineering"
	medical
		icon = 'icons/goonstation/obj/doors/doormed.dmi'
		req_access_txt = "access_medbay"
	command
		icon = 'icons/goonstation/obj/doors/doorcom.dmi'
		req_access_txt = "you_need_to_set_this"
	maintenance
		icon = 'icons/goonstation/obj/doors/doormaint.dmi'
		req_access_txt = "access_maintenance"
/*
...
/obj/machinery/door/airlock/proc/updateIconState()
	src.overlays = null
	if(src.blocked)
		var/obj/overlay/ovl = new
		ovl.icon = icon
		ovl.icon_state = "welded"
		src.overlays += ovl
	src.icon_state = text("[]door[]", (src.p_open ? "o_" : null), (density ? "_closed" : "_open"))
	return
...
/obj/machinery/door/airlock/proc/hack(mob/user as mob)
	if (src.aiHacking==0)
		...
		spawn(20)
			...
			sleep(50)
			if (src.canAIControl())
				user << "Alert cancelled. Airlock control has been restored without AI intervention."
				src.aiHacking=0
				return
...
/obj/machinery/door/airlock/open()
	if (src.blocked || src.locked || (!src.arePowerSystemsOn()) || (stat & NOPOWER) || src.isWireCut(AIRLOCK_WIRE_OPEN_DOOR))
		return 0
	use_power(AIRLOCK_POWER)
