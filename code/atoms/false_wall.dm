...
/turf/simulated/wall/false_wall/New()
	..()
...
/turf/simulated/wall/false_wall/proc/setFloorUnderlay(FloorIcon, FloorIcon_State, Floor_Intact, Floor_Health, Floor_Burnt, Floor_Name)
...
/turf/simulated/wall/false_wall/attack_paw(mob/user as mob)
	if ((ticker && ticker.mode.name == "monkey"))
		return src.attack_hand(user)
	return

/turf/simulated/wall/false_wall/attack_hand(mob/user as mob)
...
/turf/simulated/wall/false_wall/attackby(obj/item/weapon/screwdriver/S as obj, mob/user as mob)
	src.add_fingerprint(user)
	var/known = (user in known_by)
...
/turf/simulated/wall/false_wall/proc/open()
	if (src.operating)
		return 0
	src.operating = 1
	src.name = "false wall"
...
/turf/simulated/wall/false_wall/proc/close()
	if (src.operating)
		return 0
	src.operating = 1
	src.name = "wall"
...
/turf/simulated/wall/false_wall/examine()
	set src in oview(1)
	if (src.density) //door is closed
		..()
	else
