...
/obj/item/syndicate_uplink/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (get_dist(src, usr) <= 1 && istype(src.loc, /turf))))
		...
		if (href_list["item_emag"])
			...
			var/obj/item/syndicate_uplink/R = src
			R.loc = T
			T.loc = usr
...
/obj/item/baton/attack(mob/M as mob, mob/user as mob)
	src.add_fingerprint(user)
	var/mob/human/H = M
...
/obj/item/pen/sleepypen/New()
	...
	src.chem.init(/datum/reagent/sleep_toxin, 5)
	..()
...
/obj/item/pen/sleepypen/attack(mob/M as mob, mob/user as mob)
	...
	if (user)
		for(var/mob/O in viewers(M, null))
			...
			//Foreach goto(57)
		var/amount = src.chem.transfer_mob(M)
...
/obj/item/paper/attack_ai(var/mob/ai/user as mob)
...
/obj/item/storage/firstaid/toxin/New()
	...
	for(var/t in 1 to 4)
		var/obj/item/syringe/S = new /obj/item/syringe( src )
		S.chem.init(/datum/reagent/antitoxins, S.chem.max_volume)
		S.icon_state = "syringe_15"
	new /obj/item/healthanalyzer( src )
	return
	...
	if ((usr.contents.Find(src) || get_dist(src, usr) <= 1 && istype(src.loc, /turf)) || (istype(usr, /mob/ai)))
		...
		if (href_list["freq"])
			...
			if (src.traitorfreq && round(src.freq * 10, 1) == round(src.traitorfreq * 10, 1))
				...
				var/obj/item/syndicate_uplink/T = src.traitorradio
				var/obj/item/radio/R = src
				R.loc = T
				T.loc = usr
				R.layer = 0
				if (usr.client)
...
/obj/item/screwdriver/attack(mob/M as mob, mob/user as mob)
	if(!istype(M, /mob))
		return
	...
	if(istype(M, /mob/human))
		var/obj/item/organ/external/affecting = M.organs["head"]
...
/obj/item/dropper/proc/update_is()
	var/t1 = round(src.chem.cur_volume)
	if (istype(src.loc, /mob))
		if (src.mode == "inject")
...
/obj/item/dropper/New()
	src.chem = new /datum/reagent_container(5)
	..()
...
/obj/item/dropper/attack(mob/M as mob, mob/user as mob)
	...
	if (!(istype(usr, /mob/human) || ticker) && ticker.mode.name != "monkey")
		...
		return
	if(!src.chem.cur_volume)
		user << "\red The dropper is empty!"
		return
	if (user)
		for(var/mob/O in viewers(M, null))
			O.show_message("\red [M] has been eyedropped with [src] by [user].", 1)
		var/datum/reagent_container/chem2 = src.chem.split_and_remove(1)
		var/amount = chem2.dropper_mob(M)
		src.update_is()
		user.show_message("\red You drop [amount] units into [M]'s eyes. The dropper now contains [src.chem.describe()].")
		src.add_fingerprint(user)
...
/obj/item/syringe/proc/update_is()
	var/t1 = round(src.chem.cur_volume, 5)
	if (istype(src.loc, /mob))
		if (src.mode == "inject")
...
/obj/item/syringe/New()
	src.chem = new /datum/reagent_container(15)
	..()
...
/obj/item/syringe/attack(mob/M as mob, mob/user as mob)
	...
	if (user)
		if (istype(M, /mob/human))
			...
			user.show_message(text("\red You inject [] units into the []. The syringe contains [] millimeters.", amount, M, src.chem.cur_volume))
	return
...
/obj/item/weldingtool/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (status == 0 && istype(W,/obj/item/screwdriver))
		...
		icon_state = "welder2"
		s_istate = "welder"
	else if (status == 2 && istype(I,/obj/item/igniter) && !I.status)
		del(I)
		status = 3
		name = "Welder/Rods/Igniter Assembly"
		icon_state = "welder3"
	else if (status == 3 && istype(W,/obj/item/screwdriver))
		var/obj/item/flamethrower/F = new /obj/item/flamethrower(user)
		if (user.r_hand == src)
			user.unequip(src)
			user.r_hand = F
		else
...
/obj/item/weldingtool/var/processing = 0

/obj/item/weldingtool/proc/process()
...
/obj/d_girders/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/sheet/metal) && isturf(src.loc) && !istype(src.loc, /turf/space))
		if (W:amount < 1)
			del(W)
			return
		...
		del(src)
		return
	else if (istype(W, /obj/item/sheet/r_metal) && isturf(src.loc) && !istype(src.loc, /turf/space))
