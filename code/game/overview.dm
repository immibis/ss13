...
/obj/machinery/computer/security/proc/drawmap(var/mob/user as mob)
		...
		for(var/wy = 1; wy <= world.maxy; wy++)
			...
			if(!T)
				...
				switch("[T.type]")
					...
					if("/turf/simulated/engine/floor")
						colour = rgb(128,128,128)
					if("/turf/simulated/wall")
						colour = rgb(96,96,96)

					if("/turf/simulated/r_wall")
						colour = rgb(128,96,96)

					if("/turf/simulated/command/floor", "/turf/simulated/command/floor/other")
						colour  = rgb(240,240,240)

					if("/turf/simulated/command/wall", "/turf/simulated/command/wall/other")
	...
	for(var/i = 0; i<icount; i++)
		imap += icon('icons/ss13/imap.dmi', "blank")
		...
		for(var/wy = 1; wy <= world.maxy; wy++)
			...
			if(!T)
				...
				switch("[T.type]")
					...
					if("/turf/simulated/floor", "/turf/simulated/engine/floor")
						var/turf_total = T.co2 + T.oxygen + T.poison + T.sl_gas + T.n2
